...
class RigidBodyRel:
    ...
    def r_OP_q(self, t, q, B_r_SP=np.zeros(3)):
        q1 = q[:self.nq1]
        qj = q[self.nq1:]

        r_OP_q = np.einsum("ijk,j->ik", self.IB_T_q(t, q), (B_r_SP - self.B2_r_S2P2))   

        r_OP_q[:, :self.nq1] += self.predecessor_body.r_OP_q(t, q1, B_r_SP=self.B1_r_S1P1)\
            + np.einsum("ijk,j->ik", self.predecessor_body.IB_T_q(t, q1), self.B1P1_T @ self.joint.P1_r_P1P2(t, qj))

        r_OP_q[:, self.nq1:] += self.predecessor_body.IB_T(t, q1) @ self.B1P1_T @ self.joint.P1_r_P1P2_q(t, qj)
    ...
    def v_P_q(self, t, q, u, B_r_SP=np.zeros(3)):
        q1 = q[:self.nq1]
        qj = q[self.nq1:]
        u1 = u[:self.nu1]
        uj = u[self.nu1:]

        v_P_q = np.einsum("ijk,j->ik", self.IB_T_q(t, q), cross3(self.B_Omega(t, q, u), B_r_SP - self.B2_r_S2P2)) - self.IB_T(t, q) @ ax2skew(B_r_SP - self.B2_r_S2P2) @ self.B_Omega_q(t, q, u)

        tmp = self.joint.P1_r_dot_P1P2(t, qj, uj)\
                + cross3(self.B1P1_T.T @ self.predecessor_body.B_Omega(t, q1, u1), self.joint.P1_r_P1P2(t, qj))

        v_P_q[:, :self.nq1] += self.predecessor_body.v_P_q(t, q1, u1, B_r_SP=self.B1_r_S1P1)\
                + np.einsum("ijk,j->ik", self.predecessor_body.IB_T_q(t, q1), self.B1P1_T @ tmp)\
                + self.predecessor_body.IB_T(t, q1) @ self.B1P1_T @ ax2skew(self.joint.P1_r_P1P2(t, qj)) @ self.B1P1_T.T @ self.predecessor_body.B_Omega_q(t, q1, u1)

        v_P_q[:, self.nq1:] += self.predecessor_body.IB_T(t, q1) @ self.B1P1_T @ (self.joint.P1_r_dot_P1P2_q(t, qj, uj) + ax2skew(self.B1P1_T.T @ self.predecessor_body.B_Omega(t, q1, u1))@ self.joint.P1_r_P1P2_q(t, qj))

        return v_P_q

    # nu of point P (instead of reimplementing, we use that v_P(u=0)=nu_P)
    def nu_P(self, t, q, B_r_SP=np.zeros(3)):
        return self.v_P( t, q, np.zeros(self.__nu), B_r_SP=B_r_SP)
