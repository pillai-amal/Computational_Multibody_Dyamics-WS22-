...
class RigidBodyRel:
    def __init__(self,                  # this body (body 2 in lecture notes)
                m,                      # mass
    ...
    def IB_T(self, t, q):
        return self.predecessor_body.IB_T(t, q[:self.nq1]) @ self.B1P1_T @ self.joint.P1P2_T(t, q[self.nq1:]) @ self.P2B2_T

    def IB_T_q(self, t, q):
        IB_T_q = np.zeros((3, 3, self.__nq))
        IB_T_q[:, :, : self.nq1] = np.einsum(
            "ijk,jl->ilk", self.predecessor_body.IB_T_q(t, q[:self.nq1]),
                                    self.B1P1_T\
                                        @ self.joint.P1P2_T(t, q[self.nq1:])\
                                            @ self.P2B2_T
        )
        IB_T_q[:, :, self.nq1 :] = np.einsum(
            "ij,jkl,km->iml", self.predecessor_body.IB_T(t, q[:self.nq1])\
                                    @ self.B1P1_T,
                                        self.joint.P1P2_T_q(t, q[self.nq1:]),
                                            self.P2B2_T
        )
    ...
    def B_Omega_q(self, t, q, u):
        ...
        P1_Omega = self.B1P1_T.T\
            @ self.predecessor_body.B_Omega(t, q[:self.nq1], u[:self.nu1])\
            + self.joint.P1_omega_P1P2(t, q[self.nq1:], u[self.nu1:])
        A_B2P1_qj = np.einsum("ijl,jk->kil", self.joint.P1P2_T_q(t, q[self.nq1:]),
                             self.P2B2_T)
        B_Omega_q = (self.joint.P1P2_T(t, q[self.nq1:]) @ self.P2B2_T).T @ P1_Omega_q

        B_Omega_q[:, self.nq1:] += np.einsum("ijk,j->ik", A_B2P1_qj, P1_Omega)
    ...
    def B_J_R(self, t, q):
        ...
        A_B2P1 = ( self.joint.P1P2_T(t, q[self.nq1:]) @ self.P2B2_T ).T
        J_R[:, : self.nu1] = A_B2P1 @ self.B1P1_T.T\
    ...
    def B_J_R_q(self, t, q):
        ...
        A_B2P2_q = np.einsum("ijk,jl->lik",
                             self.joint.P1P2_T_q(t, q[self.nq1:]), self.P2B2_T)

        B_J_R_q[:, : self.nu1, : self.nq1] = np.einsum(
            "ij,jkl->ikl", A_B2P1 @ self.B1P1_T.T,
            self.predecessor_body.B_J_R_q(t, q[:self.nq1])
        )
        B_J_R_q[:, : self.nu1, self.nq1 :] = np.einsum(
            "ijk,jl->ilk", A_B2P2_q,
            self.B1P1_T.T @ self.predecessor_body.B_J_R(t, q[:self.nq1])
        )
        B_J_R_q[:, self.nu1 :, self.nq1 :] = np.einsum(
    ...
    def B_kappa_R(self, t, q, u):
        ...
        A_B2P1 = ( self.joint.P1P2_T(t, q[self.nq1:]) @ self.P2B2_T ).T
        P1_kappa_R = self.B1P1_T.T\
            @ self.predecessor_body.B_kappa_R(t, q[:self.nq1], u[:self.nu1])\
                + self.joint.P1_kappa_R_P1P2(t, q[self.nq1:], u[self.nu1:])\
                + cross3(self.B1P1_T.T @ self.predecessor_body.B_Omega(t, q[:self.nq1], u[:self.nu1]), self.joint.P1_omega_P1P2(t, q[self.nq1:], u[self.nu1:]))
    ...
    def B_kappa_R_q(self, t, q, u):
        ...
        P1_kappa_R = self.B1P1_T.T\
            @ self.predecessor_body.B_kappa_R(t, q[:self.nq1], u[:self.nu1])\
                + self.joint.P1_kappa_R_P1P2(t, q[self.nq1:], u[self.nu1:])\
                + cross3(self.B1P1_T.T @ pred_B_Omega , self.joint.P1_omega_P1P2(t, q[self.nq1:], u[self.nu1:]))
            
        B_kappa_R_q[:, : self.nq1] = A_B2P1\
            @ (self.B1P1_T.T @ self.predecessor_body.B_kappa_R_q(t, q[:self.nq1], u[:self.nu1]) \
                - ax2skew(self.joint.P1_omega_P1P2(t, q[self.nq1:], u[self.nu1:])) @  self.B1P1_T.T @ self.predecessor_body.B_Omega_q(t, q[:self.nq1], u[:self.nu1]))
        B_kappa_R_q[:, self.nq1 :] = A_B2P1\
            @ (self.joint.P1_kappa_R_P1P2_q(t, q[self.nq1:], u[self.nu1:]) + ax2skew(self.B1P1_T.T @ pred_B_Omega) @ self.joint.P1_omega_P1P2_q(t, q[self.nq1:], u[self.nu1:]))\
            + np.einsum("ji,kjl,k->il", self.P2B2_T, self.joint.P1P2_T_q(t, q[self.nq1:]), P1_kappa_R)
    ...
    def B_kappa_R_u(self, t, q, u):
        ...
        B_kappa_R_u[:, : self.nu1] = A_B2P1\
            @ (self.B1P1_T.T @ self.predecessor_body.B_kappa_R_u(t, q[:self.nq1], u[:self.nu1])\
                + ax2skew(self.joint.P1_omega_P1P2(t, q[self.nq1:], u[self.nu1:])) @ self.B1P1_T.T @ self.predecessor_body.B_J_R(t, q[:self.nq1]) )
        B_kappa_R_u[:, self.nu1 :] = A_B2P1\
            @ (self.joint.P1_kappa_R_P1P2_u(t, q[self.nq1:], u[self.nu1:]) + ax2skew(self.B1P1_T.T @ self.predecessor_body.B_Omega(t, q[:self.nq1], u[:self.nu1])) @ self.joint.P1_J_R_P1P2(t, q[self.nq1:]) )
        return B_kappa_R_u
    ...
    def r_OP(self, t, q, B_r_SP=np.zeros(3)):
        return self.predecessor_body.r_OP(t, q[:self.nq1], B_r_SP=self.B1_r_S1P1)\
            + self.predecessor_body.IB_T(t, q[:self.nq1]) @ self.B1P1_T @ self.joint.P1_r_P1P2(t, q[self.nq1:])\
    ...
    def v_P(self, t, q, u, B_r_SP=np.zeros(3)):
        v_P2 = self.predecessor_body.v_P(t, q[:self.nq1], u[:self.nu1], B_r_SP=self.B1_r_S1P1) \
                + self.predecessor_body.IB_T(t, q[:self.nq1]) @ self.B1P1_T @ \
                    (self.joint.P1_r_dot_P1P2(t, q[self.nq1:], u[self.nu1:])
                + cross3(self.B1P1_T.T @ self.predecessor_body.B_Omega(t, q[:self.nq1], u[:self.nu1]), self.joint.P1_r_P1P2(t, q[self.nq1:])))
    ...
    def J_P(self, t, q, B_r_SP=np.zeros(3)):
        ...
        pred_IB_T = self.predecessor_body.IB_T(t, q[:self.nq1])
        ...
        J_P[:, :self.nu1] += self.predecessor_body.J_P(t, q[:self.nq1], B_r_SP=self.B1_r_S1P1) \
            - pred_IB_T @ self.B1P1_T @ ax2skew(self.joint.P1_r_P1P2(t, q[self.nq1:])) @ self.B1P1_T.T @ self.predecessor_body.B_J_R(t, q[:self.nq1])
        J_P[:, self.nu1:] += pred_IB_T @ self.B1P1_T @ self.joint.P1_J_P1P2(t, q[self.nq1:])
        return J_P
    ...
    def J_P_q(self, t, q, B_r_SP=np.zeros(3)):
        ...
        B_r_P2P = B_r_SP - self.B2_r_S2P2
        pred_IB_T = self.predecessor_body.IB_T(t, q[:self.nq1])
        pred_IB_T_q = self.predecessor_body.IB_T_q(t, q[:self.nq1])
        ...
        J_P_q[:,
              ...
              : self.nq1] -= np.einsum(
                "ij,jlk->ilk",
              pred_IB_T @ self.B1P1_T @ ax2skew(self.joint.P1_r_P1P2(t, q[self.nq1:])) @ self.B1P1_T.T , self.predecessor_body.B_J_R_q(t, q[:self.nq1]))

        J_P_q[:,
              : self.nu1,
              : self.nq1] -= np.einsum(
                "ijk,jl->ilk",
              pred_IB_T_q, self.B1P1_T @ ax2skew(self.joint.P1_r_P1P2(t, q[self.nq1:])) @ self.B1P1_T.T @ pred_B_J_R)

        J_P_q[:,
              : self.nu1,
               self.nq1 :] -= np.einsum(
                "ijk,jl->ilk",
              pred_IB_T @ self.B1P1_T @ ax2skew_a() @ self.joint.P1_r_P1P2_q(t, q[self.nq1:]), self.B1P1_T.T @ pred_B_J_R)

        J_P_q[:,
              self.nu1 :,
              : self.nq1] += np.einsum(
            "ijk,jl->ilk", pred_IB_T_q,
            self.B1P1_T @ self.joint.P1_J_P1P2(t, q[self.nq1:])
        )
        J_P_q[:,
              self.nu1 :,
              self.nq1 :] += np.einsum(
            "ij,jlk->ilk",
            pred_IB_T @ self.B1P1_T,
    ...
    def kappa_P(self, t, q, u, B_r_SP=np.zeros(3)):
        ...
        P1_omega_IP1 = self.B1P1_T.T @ self.predecessor_body.B_Omega(t, q[:self.nq1],u[:self.nu1])
        ...
        kappa_P = self.predecessor_body.kappa_P(t, q[:self.nq1], u[:self.nu1], B_r_SP=self.B1_r_S1P1) \
            + self.predecessor_body.IB_T(t, q[:self.nq1]) @ self.B1P1_T\
                    @ (self.joint.P1_kappa_P1P2(t, q[self.nq1:], u[self.nu1:]) \
                        + cross3(self.B1P1_T.T @ self.predecessor_body.B_kappa_R(t, q[:self.nq1], u[:self.nu1]), self.joint.P1_r_P1P2(t, q[self.nq1:]))\
                        + cross3(2 * P1_omega_IP1, self.joint.P1_r_dot_P1P2(t, q[self.nq1:], u[self.nq1:]))\
    ...
    def kappa_P_q(self, t, q, u, B_r_SP=np.zeros(3)):
        ...
        B_Omega_q = self.B_Omega_q(t, q, u)
        P1_omega_IP1 = self.B1P1_T.T @ self.predecessor_body.B_Omega(t, q[:self.nq1],u[:self.nu1])
        P1_omega_IP1_q1 = self.B1P1_T.T @ self.predecessor_body.B_Omega_q(t, q[:self.nq1], u[:self.nu1])
        ...
        tmp3 = self.joint.P1_kappa_P1P2(t, q[self.nq1:], u[self.nu1:]) \
                    + cross3(self.B1P1_T.T @ pred_kappa_P_R, self.joint.P1_r_P1P2(t, q[self.nq1:]))\
                    + 2 * cross3(P1_omega_IP1, self.joint.P1_r_dot_P1P2(t, q[self.nq1:], u[self.nq1:]))\
                    + cross3(P1_omega_IP1, cross3(P1_omega_IP1, self.joint.P1_r_P1P2(t, q[self.nq1:])))

        tmp3_q1 = - ax2skew(self.joint.P1_r_P1P2(t, q[self.nq1:])) @ self.B1P1_T.T @ self.predecessor_body.B_kappa_R_q(t, q[:self.nq1], u[:self.nu1])\
                  - 2 * ax2skew(self.joint.P1_r_dot_P1P2(t, q[self.nq1:], u[self.nq1:])) @ P1_omega_IP1_q1\
        ...
        tmp3_qj = self.joint.P1_kappa_P1P2_q(t, q[self.nq1:], u[self.nu1:]) \
                    + ax2skew(self.B1P1_T.T @ pred_kappa_P_R) @ self.joint.P1_r_P1P2_q(t, q[self.nq1:])\
                    + 2 * ax2skew(P1_omega_IP1) @ self.joint.P1_r_dot_P1P2_q(t, q[self.nq1:], u[self.nq1:])\
                    + ax2skew(P1_omega_IP1) @ ax2skew(P1_omega_IP1) @ self.joint.P1_r_P1P2_q(t, q[self.nq1:])

        kappa_P_q[:, : self.nq1] +=  np.einsum("ijk,j->ik", self.predecessor_body.IB_T_q(t, q[:self.nq1]),
    ...
    def kappa_P_u(self, t, q, u, B_r_SP=np.zeros(3)):
        ...
        P1_omega_IP1 = self.B1P1_T.T @ self.predecessor_body.B_Omega(t, q[:self.nq1],u[:self.nu1])
        P1_omega_IP1_u1 = self.B1P1_T.T @ self.predecessor_body.B_J_R(t, q[:self.nq1])
        pred_IB_T = self.predecessor_body.IB_T(t, q[:self.nq1])
        ...
        tmp3_u1 = - ax2skew(self.joint.P1_r_P1P2(t, q[self.nq1:])) @ self.B1P1_T.T @ self.predecessor_body.B_kappa_R_u(t, q[:self.nq1], u[:self.nu1])\
                    - 2 *ax2skew(self.joint.P1_r_dot_P1P2(t, q[self.nq1:], u[self.nq1:])) @ P1_omega_IP1_u1\
