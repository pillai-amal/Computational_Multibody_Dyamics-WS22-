...
if __name__ == "__main__":
    ...
    for k in tqdm(range(nt-1)):
        ...
        # (c)
        # position level
        def f(qk1): # define cost function 
            error_P1 = 0.5 * np.linalg.norm(r_OP1_g(tk1) - upper_arm.r_OP(tk1, qk1[upper_arm.qDOF], B_r_SP=B1_r_S1P1))**2
            error_P2 = 0.5 * np.linalg.norm(r_OP2_g(tk1) - upper_arm.r_OP(tk1, qk1[upper_arm.qDOF], B_r_SP=B1_r_S1P2))**2
            error_P3 = 0.5 * np.linalg.norm(r_OP3_g(tk1) - upper_arm.r_OP(tk1, qk1[upper_arm.qDOF], B_r_SP=B1_r_S1P3))**2
            error_P4 = 0.5 * np.linalg.norm(r_OP4_g(tk1) - lower_arm.r_OP(tk1, qk1[lower_arm.qDOF], B_r_SP=B2_r_S2P4))**2
            error_P5 = 0.5 * np.linalg.norm(r_OP5_g(tk1) - lower_arm.r_OP(tk1, qk1[lower_arm.qDOF], B_r_SP=B2_r_S2P5))**2
            return error_P1 + error_P2 + error_P3 + error_P4 + error_P5

        sol = minimize(f, q[k], tol=1e-8) # solve minimization problem for qk1
        qk1 = sol.x
        ...
        # inverse dynamics
        W_tau = system.W_tau(tk1, qk1)
        W_tau_pinv = np.linalg.pinv(W_tau)
        M = system.M(tk1, qk1)
        h_bar = system.h_bar(tk1, qk1, uk1)
        tau[k+1] = W_tau_pinv @ (M @ u_dotk1 - h_bar)
    ...
    #########
    # (g)
    # remark: for k=0, we do not know the velocity and hence set it to zero. Consequently, when computing the acceleration for k=1, an unphysical acceleration results. Hence, the accelerations and forces are only meaningful for k>1.
    plt.plot(t[2:], tau[2:, 0], label='shoulder')
    plt.plot(t[2:], tau[2:, 1], label='biceps')
    plt.xlabel('t')
    plt.ylabel('force')
    plt.title("Forces")
    plt.legend()
