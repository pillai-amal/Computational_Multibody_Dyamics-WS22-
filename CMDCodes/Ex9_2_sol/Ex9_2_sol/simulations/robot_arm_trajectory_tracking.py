...
if __name__ == "__main__":
    ...
    for k in tqdm(range(nt-1)):
        ...
        # inverse kinematics
        # position level
        qk1 = q[k] + dt * system.q_dot(t[k], q[k], u[k])
        # velocity level
        J_A = RB2.J_P(tk1, qk1[RB2.qDOF], B_r_SP=B2_r_S2A)
        J_A_pinv = np.linalg.pinv(J_A)
        nu_A = RB2.nu_P(tk1, qk1[RB2.qDOF], B_r_SP=B2_r_S2A)
        uk1 = J_A_pinv @ (r_OA_g_dot(tk1) - nu_A)
        # acceleration level
        kappa_A = RB2.kappa_P(tk1, qk1[RB2.qDOF], uk1[RB2.uDOF], B_r_SP=B2_r_S2A)
        u_dotk1 = J_A_pinv @ (r_OA_g_ddot(tk1) - kappa_A)
        # u_dotk1 = (uk1 - u[k]) / dt
